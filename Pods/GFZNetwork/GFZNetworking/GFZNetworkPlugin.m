                          ...
                          request:(GFZNetworkRequest *)request
{
    ...
    if (request.finalParameters[KGFZUploadAppsFlyerStatisticsKey]) {
        if ([GFZNetworkConfig sharedInstance].closeStatisticsPrintfLog) return;
    }
    ...
    uploadInfo[@"request"]          = request.finalParameters;
                               ...
                               request:(GFZNetworkRequest *)request
{
    ...
    NSString *requestKey    = @"self.parmatersMd5SignHeaderKey";
    NSString *requestValue  = @"self.parmatersMd5SignHeaderValue";
    ...
    NSString *logBody = [NSString stringWithFormat:@"\n%@请求接口地址 %@= %@\n请求参数json=\n%@加密key===========%@\n加密Value=========%@\n \n %@返回= \n ",
                         successFlag, hostTitle, request.requestUrl, requestJson,
                         requestKey, requestValue, statusString];
    return logBody;
}
