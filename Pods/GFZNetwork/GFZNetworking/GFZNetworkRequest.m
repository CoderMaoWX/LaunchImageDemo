    ...
    switch (type) {
        ...
        case GFZNetworkRequestDidCompletion: {
            ...
            if (!responseModel.isCacheData) {
                [self saveResponseObjToCache:responseModel.responseDict];
            }
        }
            break;
        default:
...
- (void)readRequestCacheWithBlock:(void(^)(NSDictionary *))fetchCacheBlock {
    if (!self.autoCacheResponse) return;
    YYDiskCache *cache = [GFZNetworkConfig sharedInstance].networkDiskCache;
    [cache objectForKey:self.cacheKey withBlock:^(NSString * _Nonnull key, NSDictionary * _Nonnull cacheObject) {
...
- (void)saveResponseObjToCache:(NSDictionary *)responseObject {
    if (!self.autoCacheResponse) return;
    if (![responseObject isKindOfClass:[NSDictionary class]]) return;
    YYDiskCache *cache = [GFZNetworkConfig sharedInstance].networkDiskCache;
    [cache setObject:responseObject forKey:self.cacheKey withBlock:^{
    }];
}
